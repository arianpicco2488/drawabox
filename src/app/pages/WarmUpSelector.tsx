...
export default function WarmUpSelector() {
  ...
  const [wheelSize, setWheelSize] = useState(() =>
    window.matchMedia('(min-width: 768px)').matches ? 420 : 288
  );

  useEffect(() => {
    const mq = window.matchMedia('(min-width: 768px)');
    const handler = (e: MediaQueryListEvent) => setWheelSize(e.matches ? 420 : 288);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-md md:max-w-5xl mx-auto min-h-screen relative">
        <AppHeader
          title="Warm-Up"
        ...
        <div className="px-5 md:px-8 py-6 md:flex md:gap-8 md:items-start">
          {hasCompletedLessons ? (
            <div className="flex-shrink-0 flex flex-col items-center">
              <RouletteWheel exercises={wheelExercises} onResult={handleResult} size={wheelSize} />
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-14 px-6 rounded-2xl border border-dashed border-border bg-card/60 md:flex-1">
          ...
          {selectedExercise && (
            <div className="md:flex-1 md:self-start px-5 md:px-0 pb-8 mt-6 md:mt-0">
